...
Explorer configuration follows the same pattern. The module computes the final Explorer database settings in `compute.tf`, then renders `TFE_EXPLORER_DATABASE_*` environment variables into both the Docker Compose and Podman manifests. If Explorer is enabled without dedicated Explorer database inputs, the module falls back to the primary TFE database connection values and emits a warning output so that this non-production configuration is visible in Terraform.
